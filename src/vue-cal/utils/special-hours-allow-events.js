 ...
 */
export const eventRangeViolatesAllowEvents = ({ start, end, schedule, disallowed, hasSchedules }) => {
 ...
 */
export const maxValidEndForFixedStart = (start, endCandidate, schedule, disallowed, hasSchedules) => {
  if (!disallowed?.hasAny) return endCandidate
  const sMs = start.getTime()
  const eMs = endCandidate.getTime()
  if (eMs <= sMs) return endCandidate
  if (!eventRangeViolatesAllowEvents({ start, end: endCandidate, schedule, disallowed, hasSchedules }))
    return endCandidate
  ...
  while (lo <= hi) {
    ...
    if (!eventRangeViolatesAllowEvents({ start, end: new Date(mid), schedule, disallowed, hasSchedules })) {
      best = mid
      lo = mid + 1
    }
    else hi = mid - 1
 ...
 */
export const minValidStartForFixedEnd = (end, startCandidate, schedule, disallowed, hasSchedules) => {
  if (!disallowed?.hasAny) return startCandidate
  const eMs = end.getTime()
  const sMs = startCandidate.getTime()
  if (eMs <= sMs) return startCandidate
  if (!eventRangeViolatesAllowEvents({ start: startCandidate, end, schedule, disallowed, hasSchedules }))
  ...
  while (lo <= hi) {
    ...
    if (!eventRangeViolatesAllowEvents({ start: new Date(mid), end, schedule, disallowed, hasSchedules })) {
      ans = mid
      hi = mid - 1
    }
    else lo = mid + 1
...
export const clampDragCreateDayMinutes = ({
  ...
  const start = at(low)
  const end = at(high)
  if (!eventRangeViolatesAllowEvents({ start, end, schedule, disallowed, hasSchedules })) return { low, high }
...
export const clampResizeProposedRange = ({
  ...
  if (!eventRangeViolatesAllowEvents({ start: s, end: e, schedule, disallowed, hasSchedules })) return { start: s, end: e }
  ...
  if (!eventRangeViolatesAllowEvents({ start: s, end: e2, schedule, disallowed, hasSchedules })) return { start: s, end: e2 }
