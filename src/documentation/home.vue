...
.w-flex
  ...
  main
    ...
    .announcement.announcement--stable.w-flex.align-center
      w-icon.mr4(size="50" color="success") mdi mdi-rocket-launch-outline
      w-alert.message-box(border-left color="success-dark2")
        strong.title4 Vue Cal 5 stable is on the way!
        p.mt2.mb2 We're putting the finishing touches on the first v5 stable release after an incredible RC run.
        ul.mt0.mb2
          li #[strong Timezone support] — schedule confidently across regions.
          li #[code event.patch()] — update one event in place when your API streams changes.
          li #[strong Smarter API-driven updates] — merge by #[code id] and re-render only what changed when possible.
        p.mb0
          | Still on #[code vue-cal@next] for now. Enjoying Vue Cal?
          a.mx1(
            href="https://github.com/sponsors/antoniandre"
            target="_blank"
            style="text-decoration: underline") #[strong Sponsor on GitHub]
          | — it helps us ship faster. Thank you!

    .announcement.announcement--planisto.w-flex.align-center
      .planisto-logo-wrap.mr5.no-shrink
        img.planisto-logo(src="@/documentation/assets/planisto-logo.svg" alt="Planisto.io")
      w-alert.message-box.grow(border-left color="pale-blue")
        .title4
          strong Planisto.io — The scheduling backend you don't have to build
          w-tag.text-bold.ml2(color="info" outline round sm)
            span COMING SOON
        p.mt2.mb3.
          Vue Cal is the calendar UI. Planisto is the infrastructure behind it — API, storage, teams,
          booking, embeds — by the same author, always on the latest Vue Cal.
        w-button.ml-2(
          route="https://planisto.io"
          target="_blank"
          color="info"
          round
          text)
          strong Visit Planisto.io
    ...
    .announcement.w-flex.align-center
      ...
      w-alert.message-box.ml-1(border-left color="pale-blue")
        .text-bold
          ...
          w-button.ml-2(
            route="https://antoniandre.github.io/wave-ui"
            target="_blank"
            style="color: #1471b8"
            round
            text)
